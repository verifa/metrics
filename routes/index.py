...
table_rolling_time_individual = px.scatter(
...
table_rolling_time_individual.update_layout(title="Rolling 7 days")


# =========================================================
# Table: Rolling time (team)
# =========================================================
...
table_rolling_time_team = px.scatter(
...
table_rolling_time_team.update_layout(
...
table_rolling_income_individual.update_layout(title="Rolling 7 days (income)")


# =========================================================
# Table: Rolling income (team)
# =========================================================
...
d_team_time_rolling_30 = d_team_time_rolling_30.merge(d_team_time_rolling_7, on=["Date"])
table_rolling_income_team = px.scatter(
    d_team_time_rolling_30,
    x="Date",
    y=["Income", "Income30"],
...
table_rolling_income_team.update_layout(
...
table_rolling_total = px.scatter(
...
table_rolling_total.update_layout(
    yaxis_title="Income (euro)",
    title="Weekly income",
    xaxis_rangeslider_visible=True,
    xaxis_range=[ROLLING_DATE, str(date.today())],
...
# =========================================================
# Table: Projects (individual)
# =========================================================


d_by_group = data.byGroup().sort_values("Group")
table_projects_individual = px.histogram(
...
table_projects_individual.update_layout(bargap=0.1, title="What do we work on")
...
d_by_group = data.byGroup().sort_values("Group")
table_projects_team = px.histogram(
    d_by_group[d_by_group["Date"] > ROLLING_DATE], x="Date", y="Time", color="Group", height=600
)
table_projects_team.update_layout(bargap=0.1, title="What do we work on")
...
table_billable = px.histogram(data.data.sort_values("Key"), x="User", y="Billable", color="Key", height=600)

table_billable.update_xaxes(categoryorder="total ascending")
...
table_internal = px.histogram(data.data.sort_values("Key"), x="User", y="Internal", color="Key", height=600)

table_internal.update_xaxes(categoryorder="total descending")


# =========================================================
# Table: Popular projects
# =========================================================


table_popular_projects = px.histogram(data.data, x="Group", y="Time", color="User", height=600)

table_popular_projects.update_xaxes(categoryorder="total ascending")
...
if supplementary_data.rates.empty:
    ...
    table_eggbaskets = px.histogram(data.byTotalGroup(days_ago), x="Group", y="Billable", color="User")
    table_eggbaskets.update_xaxes(categoryorder="total ascending")
else:
    yAxisTitle = "Sum of Income (Euro)"
    table_eggbaskets = px.histogram(
        data.byEggBaskets(),
        x="Group",
        y="Income",
        color="User",
...
table_eggbaskets.update_layout(
...
figure_tabs = {
    ...
    "rates": [table_rates],
    "billable": [table_billable],
    "internal": [table_internal],
    "popular_projects": [table_popular_projects],
    "projects": [table_projects_team, table_projects_individual],
    "eggbaskets": [table_eggbaskets],
    "rolling_time": [table_rolling_time_team, table_rolling_time_individual],
    "rolling_income": [table_rolling_income_team, table_rolling_income_individual],
    "rolling_total": [table_rolling_total],
}
