...
def teamRollingAverage7(r7, tomean):
    dailyAverage = r7.groupby(["Date"])[tomean].mean()
    return dailyAverage.reset_index(inplace=False)


def rollingAverage(dailyData, tomean, days):
    monthlyAvg = dailyData.set_index("Date").rolling(str(days) + "d")[tomean].mean()
...
rolling7.loc[rolling7["Date"] < FIRST_WEEK, "Billable"] = np.nan
rolling7.loc[rolling7["Date"] < FIRST_WEEK, "Internal"] = np.nan
...
teamRollingAverage30 = rollingAverage(rollingAverage7, ["Billable", "Internal"], 30)
teamRollingAverage30.columns = ["Date", "Billable30", "Internal30"]
teamRollingAverage30.loc[teamRollingAverage30["Date"] < FIRST_MONTH, "Billable30"] = np.nan
teamRollingAverage30.loc[teamRollingAverage30["Date"] < FIRST_MONTH, "Internal30"] = np.nan
teamRollingAverage30 = teamRollingAverage30.merge(rollingAverage7, on=["Date"])
...
rolling7.loc[rolling7["Date"] < FIRST_WEEK, "Income"] = np.nan
...
teamRollingAverage30.loc[teamRollingAverage30["Date"] < FIRST_MONTH, "Income30"] = np.nan
teamRollingAverage30 = teamRollingAverage30.merge(rollingAverage7, on=["Date"])
rollingTeamAverageIncome = px.scatter(
    teamRollingAverage30, x="Date", y=["Income", "Income30"], color_discrete_sequence=["#8FBC8F", "#006400"], height=600
)
...
teamRollingAverage30.columns = ["Date", "Income30"]
teamRollingAverage30 = teamRollingAverage30.merge(rollingIncome7, on=["Date"])
teamRollingAverage90 = rollingAverage(rollingIncome7, "Income", 90)
teamRollingAverage90.columns = ["Date", "Income90"]
teamRollingAverage90 = teamRollingAverage90.merge(teamRollingAverage30, on=["Date"])
teamRollingAverage365 = rollingAverage(rollingIncome7, "Income", 365)
teamRollingAverage365.columns = ["Date", "Income365"]
teamRollingAverage365 = teamRollingAverage365.merge(teamRollingAverage90, on=["Date"])

teamRollingAverage = teamRollingAverage365
teamRollingAverage.loc[teamRollingAverage["Date"] < FIRST_WEEK, "Income"] = np.nan
teamRollingAverage.loc[teamRollingAverage["Date"] < FIRST_MONTH, "Income30"] = np.nan
teamRollingAverage.loc[teamRollingAverage["Date"] < FIRST_QUARTER, "Income90"] = np.nan
teamRollingAverage.loc[teamRollingAverage["Date"] < FIRST_YEAR, "Income365"] = np.nan
...
time3.update_layout(bargap=0.1, title="What do we work on")

# Projects team
time4data = work.byGroup().sort_values("Group")
time4 = px.histogram(time4data[time4data["Date"] > ROLLING_DATE], x="Date", y="Time", color="Group", height=600)
time4.update_layout(bargap=0.1, title="What do we work on")

# Billable time
billable = px.histogram(work.data.sort_values("Key"), x="User", y="Billable", color="Key", height=600)

billable.update_xaxes(categoryorder="total ascending")

# Internal time
internal = px.histogram(work.data.sort_values("Key"), x="User", y="Internal", color="Key", height=600)

internal.update_xaxes(categoryorder="total descending")

# Popular projects
fig3 = px.histogram(work.data, x="Group", y="Time", color="User", height=600)
...
eggbaskets.update_layout(
    height=600, yaxis_title=yAxisTitle, bargap=0.1, title="Baskets for our eggs (" + str(daysAgo) + " days back)"
)
